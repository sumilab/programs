...
//        	System.out.println("plabel2var:"+plabel2var);
    ...
    LirNode updatePhiArgs(LirNode phi, BasicBlk blk, HashMap<Integer, LirNode> phiArgMap) {
    	HashMap<LirNode,HashMap<Label,LirNode>> var2pmap = pcc.get(blk.id);
    	HashMap<Label,LirNode> plabel2var = new HashMap<Label,LirNode>();
...
//						System.out.println("Find pred var @ sameExpVarFlow!!"+predVar);
    ...
    LirNode insertPhi(LirNode node, BasicBlk blk, HashMap<Integer, LirNode> phiArgMap) {
    	...
    	phi = updatePhiArgs(phi, blk, phiArgMap);
...
//        		System.out.println("answer: top");
    	...
    	if(trues.size() > 0) {
    		if(falses.size() > 0) {
    			boolean dsafe = checkDSafety(falses, p2n) &&  checkDSafety(tops, p2n);;
        		if(dsafe) {
...
//    		else if(tops.size() > 0 && notLoop(trues, tops)) {
        		...
        		pdata.add(phi);
        		pdata.add(v);
        		laterPhiInserts.add(pdata);
            	insertPhiBlk[v.id] = true;
        		sameExpVars[v.id] = phi.kid(0);
    		}
    		else {
    			answers = lattice.True;
    ...
    boolean checkDSafety(ArrayList<BasicBlk> blks, HashMap<BasicBlk, LirNode> p2n) {
    	boolean dsafe = true;
		for(int i=0;i<blks.size();i++) {
			BasicBlk p = blks.get(i);
...
//	   printBBs();
	   for(int i=0;i<laterPhiInserts.size();i++) {
		   ...
		   LirNode phi = (LirNode)pdata.get(1);
		   BasicBlk blk = (BasicBlk)pdata.get(2);
...
//	   System.exit(2);
   ...
   void propagate(LirNode node, BasicBlk v, BiLink pp) {
	   ...
	   latestPhi = null;
	   laterPhiInserts = new ArrayList();
	   
	   int answer = NAvail(node,v);
	   
	   laterPhiInsertion(node);
